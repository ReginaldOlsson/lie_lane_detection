#include "lie_lane_detection/synthetic_bev_generator.hpp"
...
SyntheticBEVGenerator::SyntheticBEVGenerator(const PipelineParams & params)
: params_(params)
{
}
  ...
  const SyntheticSceneSpec & spec) const
{
  ...
  for (size_t i = 1; i < poly.size(); ++i) {
    cv::line(
      ...
      spec.lane_color, spec.lane_thickness);
  }
}
...
void SyntheticBEVGenerator::drawOcclusionBar(
  cv::Mat & canvas, int y0, int y1, int x0, int x1) const
{
...
GeneratedScene SyntheticBEVGenerator::generate(
  ...
  const SyntheticSceneSpec & spec_in) const
{
  ...
  switch (scenario) {
    ...
    case SyntheticScenario::DashedLanes: {
        ...
        cv::arrowedLine(
          scene.image, cv::Point(cx, by), cv::Point(cx, by - 40),
    ...
    case SyntheticScenario::IpmStyleSingle: {
        ...
        // side clutter (curbs)
        cv::rectangle(
          scene.image, cv::Point(0, 0), cv::Point(25, spec.height),
    ...
    case SyntheticScenario::CombinedChallenge: {
        ...
        drawOcclusionBar(scene.image, spec.height / 2, spec.height * 2 / 3, spec.width / 6,
          spec.width * 5 / 6);
        spec.add_noise = true;
        break;
      }
