  ...
  params.se2_vx_bins = std::max(
    ...
    std::clamp(
      static_cast<int>(std::lround(
        params.se2_vx_bins * narrow_span / std::max(full_span, 1.0))),
...
LaneTracker::LaneTracker(LaneTrackerParams params)
...
double LaneTracker::lateralStdAtY(double y, double y_max) const
{
  ...
  return params_.sigma_0_px +
         params_.sigma_alpha_px * std::pow(yn, params_.sigma_power);
}
  ...
  for (size_t i = 0; i < tracks_.size(); ++i) {
    ...
    for (size_t j = i + 1; j < tracks_.size(); ++j) {
      ...
      if (std::abs(tracks_[i].xi[0] - tracks_[j].xi[0]) < min_sep_px) {
        const bool drop_j =
          tracks_[j].misses > tracks_[i].misses ||
          tracks_[j].p_diag[0] > tracks_[i].p_diag[0];
  ...
  const PipelineParams & detect_params)
{
  ...
  for (auto & track : tracks_) {
    ...
    for (int ri = 0; ri < params_.stripe_rows; ++ri) {
      const double y =
        y_lo + (y_hi - y_lo) * static_cast<double>(ri) /
    ...
    delta[0] = clampDouble(
      delta[0],
      -params_.max_stripe_lateral_delta_px,
      params_.max_stripe_lateral_delta_px);
    delta[2] = clampDouble(
      delta[2],
      -params_.max_stripe_omega_delta,
      params_.max_stripe_omega_delta);
    delta[1] = 0.0;
    delta[3] *= 0.35;
    delta[4] *= 0.35;
...
void LaneTracker::fuseMeasurements(
  ...
  double bev_cols)
{
  ...
  std::sort(pairs.begin(), pairs.end(), [](const Assoc & a, const Assoc & b) {
  ...
  for (const auto & pair : pairs) {
    ...
    if (params_.snap_on_main) {
      ...
      track.xi_dot = (1.0 - params_.velocity_beta) * track.xi_dot +
        params_.velocity_beta * (delta / dt);
    }
    track.role = det.role;
    track.misses = 0;
  ...
  tracks_.erase(
    std::remove_if(
      ...
      [&](const LaneTrack & track) {
        return track.misses > params_.max_misses;
      }),
    tracks_.end());
  ...
  double video_fps)
{
  ...
  const bool run_main =
    tracks_.empty() ||
    (processed_frames_ % params_.main_detect_interval == 0);
